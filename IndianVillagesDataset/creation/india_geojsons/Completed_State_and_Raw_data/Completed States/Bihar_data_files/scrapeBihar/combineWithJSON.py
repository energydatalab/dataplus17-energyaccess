...
def updateJSON(knowndict, translate, state):
	...
	for village in state["features"]:
		...
		if village["properties"]["CEN_2001"] in translate:
		...
		if (village["properties"]["CEN_2011"] in knowndict):
			village["properties"]["HH"] = int(knowndict[village["properties"]["CEN_2011"]][0])
			village["properties"]["eH"] = int(knowndict[village["properties"]["CEN_2011"]][1])
			village["properties"]["perc"] = 100*float(village["properties"]["eH"])/float(village["properties"]["HH"])
			#print(village["properties"]["perc"])
			if (village["properties"]["perc"]) <= 1:
				print("NON ELECTRIFIED: " + str(village["properties"]["perc"]))
			#print(type(village["properties"]["perc"]))
			totalKnown += 1
		else:
			village["properties"]["HH"] = -9
			village["properties"]["eH"] = -9
			village["properties"]["perc"] = -9
		#print(village["properties"])
		#print ("Total: " + str(totalKnown))
	return state

def cwj (translateName, mapjson, stateName):
	translateDict = build0111Dictionary('bh_village_2011_2001_code_mapping.txt')
	knowndict = build11ElecDictionary('known.csv')
	state = json.loads(''.join(open('br.geojson','r').readlines()))
...
cwj('bh_village_2011_2001_code_mapping.txt', 'br.geojson', 'bihar')
